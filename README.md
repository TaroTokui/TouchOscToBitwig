...
- [TDBitwig](https://github.com/DBraun/TDBitwig) Controller Extension v1.1.12
...
| OUT (localhost) | localhost | 10002 | TouchDesigner → TouchOSC (同一PC) |
| OUT (リモート) | 192.168.11.12 | 10002 | TouchDesigner → TouchOSC (別デバイス) |
...
│   │   ├── button1     # トラック選択
│   │   ├── button2     # ナビゲーション
│   │   └── button3     # ON ボタン（bitwigTrack の color を反映）
...
1. Bitwig Studio に TDBitwig Controller Extension をインストール・設定
2. `TouchOscToBitwig.toe` を TouchDesigner で開く
3. `bitwigMain` の接続パラメータを環境に合わせて調整
4. `oscout2` の送信先 IP アドレスを TouchOSC デバイスの IP に変更
5. TouchOSC のレイアウトを上記 OSC フォーマットに合わせて設定
